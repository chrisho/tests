...
class StorageClassManager(BaseManager):
    ...
    def set_default(self, name, *, raw=False):
        ...
        if resp[0] == 200 and len(resp[1]['items']):
            scs = resp[1]['items']
            for k in range(len(scs)):
                if 'annotations' in scs[k]['metadata'] and \
                   DEFAULT_STORAGE_CLASS_ANNOTATION in scs[k]['metadata']['annotations'] and \
                   scs[k]['metadata']['annotations'][DEFAULT_STORAGE_CLASS_ANNOTATION] == 'true':

                    if scs[k]['metadata']['name'] == name:
                        return (200, scs[k])

                    # reset old default storage class
                    path = self.PATH_fmt.format(name=scs[k]['metadata']['name'],
